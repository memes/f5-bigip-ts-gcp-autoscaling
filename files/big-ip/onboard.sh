...
RUNTIME_INIT_URL="${RUNTIME_INIT_URL:-"https://cdn.f5.com/product/cloudsolutions/f5-bigip-runtime-init/v1.6.2/dist/f5-bigip-runtime-init-1.6.2-1.gz.run"}"

# Log an info message to stdout and ttyS0
info()
{
    echo "$0: INFO: $*" >&2
    [ -e /dev/ttyS0 ] && echo "$0: INFO: $*" >/dev/ttyS0
}

# Log an error message to stdout and ttyS0 and terminate script
error()
{
    echo "$0: ERROR: $*" >&2
    [ -e /dev/ttyS0 ] && echo "$0: ERROR: $*" >/dev/ttyS0
    exit 1
}
...
auth_token()
{
    ...
    while [ "${attempt}" -lt 10 ]; do
        auth_token="$(http_proxy='' https_proxy='' HTTP_PROXY='' HTTPS_PROXY='' curl -sf --retry 20 -H "Metdata-Flavor: Google" http://169.254.169.254/computeMetadata/v1/instance/service-accounts/default/token | jq --raw-output '.access_token')"
        retval=$?
        if [ "${retval}" -eq 0 ]; then
            echo "${auth_token}"
            break
...
wait_bigip_ready
nic_count="$(http_proxy='' https_proxy='' HTTP_PROXY='' HTTPS_PROXY='' curl -sf --retry 20 -H "Metadata-Flavor: Google" "http://169.254.169.254/computeMetadata/v1/instance/network-interfaces/?recursive=true" | jq --raw-output '.|length')"
if [ "${nic_count}" -gt 1 ] && [ ! -f /config/cloud/.mgmtInterface ]; then
    info "Getting management interface configuration from metadata"
    target_address="$(http_proxy='' https_proxy='' HTTP_PROXY='' HTTPS_PROXY='' curl -sf --retry 20 -H "Metadata-Flavor: Google" http://169.254.169.254/computeMetadata/v1/instance/network-interfaces/1/ip)"
    target_netmask="$(http_proxy='' https_proxy='' HTTP_PROXY='' HTTPS_PROXY='' curl -sf --retry 20 -H "Metadata-Flavor: Google" http://169.254.169.254/computeMetadata/v1/instance/network-interfaces/1/subnetmask)"
    target_gateway="$(http_proxy='' https_proxy='' HTTP_PROXY='' HTTPS_PROXY='' curl -sf --retry 20 -H "Metadata-Flavor: Google" http://169.254.169.254/computeMetadata/v1/instance/network-interfaces/1/gateway)"
    target_mtu="$(http_proxy='' https_proxy='' HTTP_PROXY='' HTTPS_PROXY='' curl -sf --retry 20 -H "Metadata-Flavor: Google" http://169.254.169.254/computeMetadata/v1/instance/network-interfaces/1/mtu)"
...
if [ -f /config/cloud/runtime-init-conf.yaml ]; then
    if [ -x /usr/local/bin/f5-bigip-runtime-init ]; then
        ...
        # Explicitly disable proxy vars to avoid google-auth/gaxios ignoring no_proxy
        http_proxy='' https_proxy='' HTTP_PROXY='' HTTPS_PROXY='' /usr/local/bin/f5-bigip-runtime-init \
                --config-file /config/cloud/runtime-init-conf.yaml \
                --skip-telemetry \
                ${SKIP_TELEMETRY:+--skip-telemetry}
        retval=$?
        if [ "${retval}" -ne 0 ]; then
            cat /var/log/cloud/bigIpRuntimeInit.log > /dev/ttyS0
